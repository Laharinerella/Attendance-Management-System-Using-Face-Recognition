...
def user_details(roll_number):
    ...
    # Fetch latest attendance date
    latest_date_query = "SELECT MAX(date) AS latest_date FROM attendance WHERE roll_number = %s"
    cursor.execute(latest_date_query, (roll_number,))
    latest_date_result = cursor.fetchone()
    latest_date = latest_date_result['latest_date']
    
    # Fetch highest number of total days attended
    highest_attendance_query = "SELECT MAX(total_days_attended) AS max_days_attended FROM attendance WHERE roll_number = %s"
    cursor.execute(highest_attendance_query, (roll_number,))
    highest_attendance_result = cursor.fetchone()
    max_days_attended = highest_attendance_result['max_days_attended']
    
    # Fetch total number of days attended
    total_attendance_query = "SELECT SUM(total_days_attended) AS total_days_attended FROM attendance WHERE roll_number = %s"
    cursor.execute(total_attendance_query, (roll_number,))
    total_attendance_result = cursor.fetchone()
    total_days_attended = total_attendance_result['total_days_attended']
    
    # Fetch attended dates
    attended_dates_query = "SELECT date FROM attendance WHERE roll_number = %s"
    cursor.execute(attended_dates_query, (roll_number,))
    attended_dates = [row['date'] for row in cursor.fetchall()]
    
    cursor.close()
    db_connection.close()
    
    if user and latest_date and max_days_attended is not None and total_days_attended is not None:
        return render_template('user_details.html', user=user, latest_date=latest_date, max_days_attended=max_days_attended, total_days_attended=total_days_attended, attended_dates=attended_dates)
    else:
        return "User not found or attendance record not available."
...
def registration():
    ...
    if request.method == 'POST':
        ...
        # Validate password
            ...
            error_message = "Password must contain at least 8 characters including one lowercase, one uppercase, and one special character."
        else:
            ...
            if roll_result:
                error_message = "Roll number already exists. Please try a different roll number."
            elif email_result:
    ...
    return render_template('registration.html', error_message=error_message, success_message=success_message, form_data=form_data)
...
def teacher_login():
    if request.method == 'POST':
        ...
        if teacher_id in teacher_credentials and teacher_credentials[teacher_id] == password:
            ...
            return render_template('teachers.html', error_message="Invalid credentials. Please try again.")
    else:
...
def live_recognition(camera):
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if predicted_class_probability > 0.99: 
            ...
            cv2.putText(frame, str(label), (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (36,255,12), 2)
            
            # Check if attendance has already been taken for today
            if label != "Unknown" and not attendance_taken_today(str(label)):
...
def train_data():
    def generate():
        ...
        model = Sequential([
            Conv2D(32, (3, 3), activation='relu', input_shape=(150, 150, 3)),  # Adjust input shape
            MaxPooling2D((2, 2)),
...
def find_student():
    if request.method == 'POST':
        ...
        # Fetching student details from registrations table excluding the password field
        cursor.execute("SELECT full_name, roll_number, phone_number, email, gender, department, year_of_study FROM registrations WHERE roll_number = %s", (roll_number,))
        student_details = cursor.fetchone()

        if student_details is None:
            connection.close()
            return render_template('find_student.html', error_message="Invalid roll number. Student not found.")

        # Fetching attendance details from attendance table
        cursor.execute("SELECT total_days_attended FROM attendance WHERE roll_number = %s ORDER BY total_days_attended DESC LIMIT 1", (roll_number,))
        attendance_details = cursor.fetchone()

        # Fetching attended dates
        cursor.execute("SELECT date FROM attendance WHERE roll_number = %s", (roll_number,))
        attended_dates = [row['date'] for row in cursor.fetchall()]

        connection.close()

        return render_template('student_details.html', student=student_details, attendance=attendance_details, attended_dates=attended_dates)
    else:
